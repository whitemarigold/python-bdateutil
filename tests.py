...
class TestRelativeDelta(unittest.TestCase):
    def test_init(self):
        self.assertEqual(
            relativedelta(date(2014, 1, 7), date(2014, 1, 3)),
            relativedelta(days=4, bdays=2),
        )
        self.assertEqual(
            relativedelta(date(2014, 1, 31), date(2014, 1, 1)),
            relativedelta(days=30, bdays=22),
        )
        self.assertEqual(
            relativedelta(date(2014, 2, 1), date(2014, 1, 1)),
            relativedelta(months=1, bdays=23),
        )
        self.assertEqual(
            relativedelta(date(2014, 2, 2), date(2014, 1, 1)),
            relativedelta(months=1, days=1, bdays=23),
        )
        self.assertEqual(
            relativedelta(date(2014, 1, 1), date(2014, 2, 2)),
            relativedelta(months=-1, days=-1, bdays=-23),
        )
    ...
    def test_add(self):
        ...
        self.assertEqual(relativedelta(bdays=3) + date(2014, 1, 3), date(2014, 1, 8))
        rd4 = relativedelta(years=+1, months=+2, days=+1)
        rd5 = relativedelta(years=+4, months=+1, bdays=+7, days=+10)
        self.assertEqual(rd3 + rd4, rd5)
        self.assertEqual("2014-01-01" + relativedelta(weekday=FR), datetime(2014, 1, 3))

    def test_radd(self):
        self.assertEqual(date(2014, 1, 3) + relativedelta(bdays=2), date(2014, 1, 7))
        self.assertEqual(date(2014, 1, 7) + relativedelta(bdays=-2), date(2014, 1, 3))
        self.assertEqual(date(2014, 2, 3) + relativedelta(bdays=-19), date(2014, 1, 7))
    ...
    def test_neg(self):
        self.assertEqual(
            -relativedelta(years=+1, bdays=-3), relativedelta(years=-1, bdays=+3)
        )
    ...
    def test_mul(self):
        self.assertEqual(
            relativedelta(years=+1, bdays=-3) * 3, relativedelta(years=+3, bdays=-9)
        )
        self.assertEqual(
            relativedelta(years=+1, bdays=-3) * -3, relativedelta(years=-3, bdays=+9)
        )
        self.assertEqual(
            relativedelta(years=+1, bdays=-3) * 0, relativedelta(years=0, bdays=0)
        )

    def test_rmul(self):
        self.assertEqual(
            3 * relativedelta(years=+1, bdays=-3), relativedelta(years=+3, bdays=-9)
        )
        self.assertEqual(
            -3 * relativedelta(years=+1, bdays=-3), relativedelta(years=-3, bdays=+9)
        )
        self.assertEqual(
            0 * relativedelta(years=+1, bdays=-3), relativedelta(years=0, bdays=0)
        )
    ...
    def test_div(self):
        self.assertEqual(
            relativedelta(years=+3, bdays=-9) / 3, relativedelta(years=+1, bdays=-3)
        )
        self.assertEqual(
            relativedelta(years=+3, bdays=-9) / -3, relativedelta(years=-1, bdays=+3)
        )
        self.assertRaises(ZeroDivisionError, lambda: relativedelta(bdays=-3) / 0)

    def test_truediv(self):
        self.assertEqual(
            relativedelta(years=+4, bdays=-10) / 3.0, relativedelta(years=+1, bdays=-3)
        )

    def test_repr(self):
        rd1 = relativedelta(years=+1, months=+2, days=-3)
        self.assertEqual(str(rd1), "relativedelta(years=+1, months=+2, days=-3)")
        rd2 = relativedelta(years=+1, months=+2, bdays=-7)
        self.assertEqual(str(rd2), "relativedelta(years=+1, months=+2, bdays=-7)")
        rd3 = relativedelta(years=-1, months=-2, bdays=+7)
        self.assertEqual(str(rd3), "relativedelta(years=-1, months=-2, bdays=+7)")
...
class TestParser(unittest.TestCase):
    ...
    def test_parserinfo(self):
        ...
        self.assertEqual(parse("1/2/2014", dayfirst=True), datetime(2014, 2, 1))
        self.assertEqual(
            parse("1/2/2014", parserinfo(dayfirst=True)), datetime(2014, 2, 1)
        )
...
class TestRRule(unittest.TestCase):
    def test_bdaily(self):
        ...
        self.assertEqual(
            list(rrule(BDAILY, count=4, dtstart=start)),
            [
    ...
    def test_parse(self):
        self.assertEqual(
            list(rrule(BDAILY, count=4, dtstart="2014-01-01")),
            [
                datetime(2014, 1, 1, 0, 0),
                datetime(2014, 1, 2, 0, 0),
                datetime(2014, 1, 3, 0, 0),
        ...
        self.assertEqual(
            list(rrule(BDAILY, count=4, dtstart="2014-01-01", until="01/04/2014")),
            [
                datetime(2014, 1, 1, 0, 0),
                datetime(2014, 1, 2, 0, 0),
                datetime(2014, 1, 3, 0, 0),
