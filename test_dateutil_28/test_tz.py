...
from bdateutil import datetime, timedelta
from bdateutil import time as dt_time
from bdateutil import tzinfo
...
# dateutil imports
from bdateutil.relativedelta import relativedelta, SU
from bdateutil.parser import parse
from bdateutil import tz as tz
...
class TzFoldMixin(object):
    ...
    def testFoldNegativeUTCOffset(self):
        ...
        with self._gettz_context(tzname):
            ...
            self.assertEqual(t0_tor0.replace(tzinfo=None), datetime(2011, 11, 6, 1, 30))

            self.assertEqual(t1_tor1.replace(tzinfo=None), datetime(2011, 11, 6, 1, 30))
...
class TZStrTest(unittest.TestCase, TzFoldMixin):
    ...
    def testStrStart1(self):
        self.assertEqual(
            datetime(2003, 4, 6, 1, 59, tzinfo=tz.tzstr("EST5EDT")).tzname(), "EST"
        )
        self.assertEqual(
            datetime(2003, 4, 6, 2, 00, tzinfo=tz.tzstr("EST5EDT")).tzname(), "EDT"
        )

    def testStrEnd1(self):
        self.assertEqual(
            datetime(2003, 10, 26, 0, 59, tzinfo=tz.tzstr("EST5EDT")).tzname(), "EDT"
        )

        end = tz.enfold(
            datetime(2003, 10, 26, 1, 00, tzinfo=tz.tzstr("EST5EDT")), fold=1
        )
    ...
    def testStrStart2(self):
        ...
        s = "EST5EDT"
        self.assertEqual(
            datetime(2003, 4, 6, 1, 59, tzinfo=tz.tzstr(s)).tzname(), "EST"
        )
        self.assertEqual(
            datetime(2003, 4, 6, 2, 00, tzinfo=tz.tzstr(s)).tzname(), "EDT"
        )

    def testStrEnd2(self):
        s = "EST5EDT"
        self.assertEqual(
            datetime(2003, 10, 26, 0, 59, tzinfo=tz.tzstr(s)).tzname(), "EDT"
        )

        end = tz.enfold(datetime(2003, 10, 26, 1, 00, tzinfo=tz.tzstr(s)), fold=1)
        self.assertEqual(end.tzname(), "EST")
    ...
    def testStrCmp1(self):
        self.assertEqual(
            tz.tzstr("EST5EDT"), tz.tzstr("EST5EDT4,M4.1.0/02:00:00,M10-5-0/02:00")
        )
